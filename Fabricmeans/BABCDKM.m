...
function [Y, minO, iter_num, sse, obj, balance_loss, elapsed_time] = Fabricmeans(X, label,c, block_size, rho, numWorkers, max_iters)
fprintf("Fabricmeans\n");
...
for iter_t = 1:max_iters
    ...
    for ii = 1:c
        ...
        d2c = sum(Xi.^2, 1) + c2 - 2 * (ceni' * Xi);  
        sumd(ii) = sum(d2c);      
    
        diff = thetas(ii) - FF(ii);
        balance_loss_OBJ(ii) = lambdas(ii) * diff + rho * diff^2;
        balance_loss_t(ii) = diff^2;
    end
...
disp(['Elapsed time: ', num2str(elapsed_time), ' seconds', ' sse, ', num2str(sse(iter_num))]);
delete(gcp('nocreate'))
